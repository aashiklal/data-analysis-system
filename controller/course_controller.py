...
def reset_database():
    try:
        User.clear_user_data(User())
        Course.clear_course_data(Course())
        render_result()
    except:
        render_err_result(msg="exception happened")
...
def course_list():
    ...
    if User.current_login_user:  # check login user
        ...
        (one_page_course_list, total_pages, total_num) = Course.get_courses_by_page(Course(), 1)

        # check one_page_course_list, make sure this variable not be None, if None, assign it to []
        if one_page_course_list is None:
